...
# mutate a variable that represents the number of paintings of a certain author during this period
npaintings <- count(paintings_train, authorstandard)
paintings_train_new <- merge(paintings_train, npaintings, by = "authorstandard")
...
paintings_train_new[which(paintings_train_new$Diam_in != 0), 61] <- 1

# replace blank space and NA of categorical variables with n/a
paintings_train_new[, c(5, 7:10, 13:28, 30:32, 34:59)] <- as.data.frame(sapply(paintings_train_new[, c(5, 7:10, 13:28, 30:32, 34:59)], function(x) ifelse(x == "" | x == "-", "n/a", x)))

# clean position
paintings_train_new <- paintings_train_new[-which(paintings_train_new[, 4] > 1),]
...
Besides, `sale`, `lot`, `subject`, `author`, `winningbidder` and `material` have too many levels, and thus they are not selected.

In addition, we have alrealy built a variable that contains the useful information of `authorstandard`, so we can drop it as well. Similarly, we can drop `Height_in`, `Width_in`, `Surface_Rect`, `Diam_in` and `Surface_Rnd`.

```{r}
# choose variables
paintings_train_new <- paintings_train_new %>% 
  select(-authorstandard, -sale, -lot, -price, -count, -subject, -author, -winningbidder, -Interm, -Height_in, -Width_in, -Surface_Rect, -Diam_in, -Surface_Rnd, -material, -materialCat)
```
...
Since the points of `Surface`, `nfigures`, `n` and `ratio` are clustered at the beginning of x axis, we can do log transformation to the corresponding predictors to see their relationship with `logprice`.
...
From the result of random forest, we can choose $17$ variables.

```{r, echo = TRUE, cache = TRUE}
# full model
model_full <- lm(logprice ~ (position + dealer + year + origin_author + origin_cat + school_pntg + winningbiddertype + endbuyer + type_intermed + Shape + Surface + mat + paired + finished + lrgfont + n + ratio)^2, data = paintings_train_new)
