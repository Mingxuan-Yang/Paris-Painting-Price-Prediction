...
2. origin_cat: without knowning the paintings before the auction, the buyers can only get to know the origin of paintings through the dealer's classification. For the well investigated painting, the dealer will classify the painting correctly. For unknown painting, neither dealer and buyer know the origin. So we think origin_cat would be helpful for our model beside origin_author. And we use an anova test to check that.
```{r}
...
3. authorstyle: For this predictor, originally there are many n/a exist. But we think n/a represent a special category. By looking at the barplot of author_style,  we can see the means of each group are differnet. So we choose this predictor to help us predict the outcome.

4.type_intermed: By intuition, we think the type of intermediary plays an important role in the sale price.
Since differnet type of intermediaries sell paintings for differnet purposes and the price will flutuate based on that. And the barplot of type_intermed can support our idea.
...
Finally, in the initial model, we have 8 main predictors and 5 interactions. Roughly 60% variation of dependent variables are explained by this model. By looking at the anova table of the model, all of the variables are significant at the 5% level, which indicate the variables in the model are reasonable.
...
According to the coefficient table we get above, predict year and endbuyer have huge impact on the price sale.
For the year, althouth its coefficient is not large compared to others, the big numeric value itself will have
impact on the price. Beside the year, for the dummy variable, endbuyer is another important predictor that affect the price most.
