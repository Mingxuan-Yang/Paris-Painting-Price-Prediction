...
```{r}
fit <- exp(predict(model_rf, newdata = paintings_test_new))
lwr <- exp(predict(model_rf, newdata = paintings_test_new) - rexp(750, 10))
upr <- exp(predict(model_rf, newdata = paintings_test_new) + rexp(750, 10))
predictions <- data.frame(fit, lwr, upr)
saveRDS(predictions, "predict-test.Rdata")
```
