...
```{r}
forest=randomForest(logprice ~ year + log_Surface + dealer + lrgfont + endbuyer + position + origin_author + finished + type_intermed, data=paintings_train_new,mtry=9,importance=TRUE)
...
for(i in 1:30){
  ...
  bootstrap_tree=randomForest(logprice ~ year + log_Surface + dealer + lrgfont + endbuyer + position + origin_author + finished + type_intermed, data=bootstrap, mtry=9, importance=TRUE)
