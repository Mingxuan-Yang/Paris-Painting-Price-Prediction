...
```{r}
# model
model_rf <- randomForest(logprice ~ year + log_Surface + dealer + lrgfont + endbuyer + origin_author + finished + winningbiddertype, data = paintings_train_new, mtry = 4, importance = TRUE)
...
validation_true <- read.csv("paris_paintings.csv")[row_names, -c(1, 34)]

row_names <- rownames(paintings_test)
row_names <- as.double(row_names)
test_true <- read.csv("paris_paintings.csv")[row_names, -c(1, 34)]
```
