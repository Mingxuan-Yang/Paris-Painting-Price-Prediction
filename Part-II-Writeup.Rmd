...
```{r}
BPM <- predict(bma2, estimator = "BPM")
variable.names(BPM)
```

```{r}
lm.BPM <- lm(logprice ~ dealer + year + origin_author + endbuyer + log_Surface + finished
             + lrgfont + type_intermed + dealer:year + year:endbuyer + year:lrgfont + year:type_intermed + log_Surface:lrgfont + finished:lrgfont, data = paintings_train_new)
```
