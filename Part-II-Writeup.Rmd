...
# coverage
coverage <- function(y, ypred){
  mean(ypred[,"lwr"]<y&ypred[,"upr"]>y)
}
...
```{r}
# model
model_rf <- randomForest(logprice~year+log_Surface+dealer+lrgfont+endbuyer+origin_author+
...
forest=randomForest(logprice~year+log_Surface+dealer+lrgfont+endbuyer+origin_author+
                      finished+type_intermed+diff_origin+
                      prevcoll, data=paintings_train_new,mtry=10,importance=TRUE)
...
for(i in 1:30){
  ...
  bootstrap_tree=randomForest(logprice~year+log_Surface+dealer+lrgfont+endbuyer+
                                origin_author+finished+type_intermed+
                                diff_origin+prevcoll,
                              data=bootstrap, mtry=10, importance=TRUE)
