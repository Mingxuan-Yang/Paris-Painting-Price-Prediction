...
for(i in 1:30){
  ...
  bootstrap_tree=randomForest(logprice ~ year + log_Surface + dealer + lrgfont + endbuyer + position + origin_author + finished + type_intermed, data=bootstrap, mtry=7, importance=TRUE)
  yhat=predict(bootstrap_tree,paintings_test_new)
  mean[i,]=yhat
}
means=apply(mean, 2, median)
