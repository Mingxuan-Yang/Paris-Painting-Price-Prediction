...
```{r cache = TRUE}
# full model
model_test <- lm(logprice ~ (dealer + year + origin_author + endbuyer + log_Surface + finished + lrgfont + position + type_intermed)^2, data = paintings_train_new)
...
```{r cache = TRUE}
set.seed(3)
bma <- bas.lm(formula(model1), paintings_train_new,
...
predictions <- predict(bma, newdata = paintings_test_new, se.fit = TRUE)
```
...
pred <- data.frame(fit, lwr, upr)
save(pred, file="predict-test.Rdata")
```
