...
paintings_train_new <- as.data.frame(sapply(paintings_train, function(x) ifelse(paste(x) == "" | paste(x) == "n/a", NA, x)))
...
set.seed(1)
train(logprice ~ dealer + year + origin_cat + diff_origin + 
    artistliving + Height_in + Width_in + Surface_Rect + engraved + 
    prevcoll + finished + lrgfont + lands_sc + discauth + dealer:discauth + 
    year:discauth + Height_in:discauth + prevcoll:finished, method = "lm", data = paintings_train_new, trControl = trainControl(method = "cv", number = 10))
```

10-fold cross validation RMSE $1.165233$.
...
paintings_test <- read.csv("paris_paintings.csv")[row_names, -c(1, 34)]

# replace blank space with NA
paintings_test_new <- as.data.frame(sapply(paintings_test, function(x) ifelse(paste(x) == "" | paste(x) == "n/a", NA, paste(x))))
...
# prediction
predictions <- exp(predict(model1, newdata = paintings_test_new))
na <- which(is.na(predictions))
rmse(predictions[-na], exp(paintings_test$logprice[-na]))
```
